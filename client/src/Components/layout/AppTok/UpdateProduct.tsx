    ...
    if (correct.name && correct.price && correct.category && correct.brand) {
      ...
      updateProduct(id, data)
        .then((data) => {
          ...
          // нужно сбросить поле загрузки изображения, но не очищать все поля формы для следующего товара, потому что при повтороном редактировании того же товара все поля окажутся пустыми
          // сбрасываем поле загрузки изображения, чтобы при сохранении товара, когда новое изображение не выбрано, не загружать старое повтороно
